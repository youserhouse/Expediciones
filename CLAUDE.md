...
- **2026-08-10 — Dos controles de edición dentro de la misma tarjeta no pueden usar `card.querySelector('.edit-btn')`:** Al agregar un pencil/save por cada expedición además del de la nota, `card.querySelector('.edit-btn')` (scoped a todo `#cierre-{id}`) empieza a devolver el primer `.edit-btn` que encuentre en el DOM, sin importar cuál botón disparó la función — mezclando el estado de la nota con el de una expedición cualquiera. **Por qué importa:** en cuanto un contenedor puede tener N controles del mismo patrón edit/save, cada `toggle*/save*` debe escopar sus queries a un wrapper propio con id único (`#hc-nota-{id}`, `#exp-{cierreId}-{idx}`), nunca al contenedor padre compartido.
...
- **Historial panels**: data loaded once into `histAllCierres` / `histAllUbicados`; client-side `filterByDate()` re-renders the filtered accordion without extra DB calls. Both panels share the same per-item edit pattern: the editable inputs start `disabled`, a pencil `.edit-btn` enables them and reveals a hidden `.save-btn`, and a `.del-btn` ("✕") removes the row. Canceling edit (clicking the pencil again) restores `data-orig` instead of persisting. **The edit controls live inside `.cierre-detail`, which is `max-height:0` until the card is expanded** — they are unreachable until the user clicks `.cierre-head`. **Facturas Anteriores has two independent, separately-scoped edit controls per cierre**: the cierre-level nota (`#hc-nota-{id}`, `toggleEditHistCierre`/`saveHistCierre`, only touches `nota`) and one control **per expedición inside `expediciones`** (`#exp-{cierreId}-{idx}`, `toggleEditExp`/`saveExp`, edits `factura`/`palets`/`ubicados`/`p_ubicar` — the actual per-invoice fields, since a cierre is almost always one invoice but can hold several after "Limpiar Pizarra"). Saving an expedición rewrites the whole `expediciones` jsonb array on that cierre (there's no separate row per invoice to update). Each control queries elements scoped to its own wrapper id, never `card.querySelector` on the whole `#cierre-{id}` — multiple `.edit-btn`/`.save-btn` coexist in one card (nota + N expediciones) and a card-wide query would grab the wrong one. Ubicados por Día edits two fields (`.ci-palets`, `.ci-nota`).
