...
- **Historial panels**: data loaded once into `histAllCierres` / `histAllUbicados`; client-side `filterByDate()` re-renders the filtered accordion without extra DB calls. Both panels share the same per-item edit pattern: the editable inputs start `disabled`, a pencil `.edit-btn` (`toggleEditHistCierre`/`toggleEditHistUbic`) enables them all and reveals a hidden `.save-btn` (`saveHistCierre`/`saveHistUbic`), and a `.del-btn` ("✕") removes the row (`deleteHistorialItem`/`deleteHistUbic`). Canceling edit (clicking the pencil again) restores `data-orig` instead of persisting. **The edit controls live inside `.cierre-detail`, which is `max-height:0` until the card is expanded** — they are unreachable until the user clicks `.cierre-head`. Facturas Anteriores edits four fields (`.ci-hc-fact`, `.ci-hc-pal`, `.ci-hc-pend`, `.ci-nota`, listed in the `HC_FIELDS` selector); Ubicados por Día edits two (`.ci-palets`, `.ci-nota`).
...
- **Facturas Anteriores: the three header totals are now editable too** — previously the cierre detail only exposed an editable `nota`, so the `N facturas · N palets · N pendientes` figures shown in the header could not be corrected after archiving. The detail now opens with a `.hc-fields` row of three number inputs (`.ci-hc-fact`, `.ci-hc-pal`, `.ci-hc-pend`) that the same pencil unlocks alongside the nota; saving persists all four in one update, clamps `total_pendientes` to `[0, total_palets]`, normalizes negatives/blanks to 0, and refreshes the header `<b>` values in place (ids `hc-fact-{id}` / `hc-pal-{id}` / `hc-pend-{id}`) without a full re-render.
