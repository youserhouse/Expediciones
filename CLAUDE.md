...
- **Configuración has 3 accordion sections** (`renderCfgPanel()`, state in `cfgOpenSections`): **KPIs** (a "Vincular colores con Estado" toggle, then order hint + per-KPI color/visibility — while linked, entries with `mirrorsEstado` show a "sigue al estado…" note instead of swatches; unlinked, all 6 KPIs behave the same), **Columnas** (color of the 3 numeric table/mobile-card fields: `Total Palets`/`Ubicados`/`P. por ubicar`, backed by `--col-palets`/`--col-ubicados`/`--col-pendientes`, each own `localStorage` key `expediciones.columnConfig.v1`), **Estado** (color of the 4 estados, `expediciones.estadoConfig.v1`). `resetAllConfig()` clears all three keys at once. The "Palés" field in Historial Ubicados por Día uses its own `.ci-diapalets` class (not `.ci-palets`) specifically so recoloring the table's Total Palets column doesn't affect it.
