...
- `id` uuid, `cerrado_at` timestamptz, `total_facturas`, `total_palets`, `total_pendientes` int, `expediciones` jsonb (full snapshot), `nota` text (free-text annotation on the whole cierre, editable from the panel)
...
- **Historial panels**: data loaded once into `histAllCierres` / `histAllUbicados`; client-side `filterByDate()` re-renders the filtered accordion without extra DB calls. Both panels share the same per-item edit pattern: a `.ci-nota` input starts `disabled`, a pencil `.edit-btn` (`toggleEditHistCierre`/`toggleEditHistUbic`) enables it and reveals a hidden `.save-btn` (`saveHistCierre`/`saveHistUbic`), and a `.del-btn` ("✕") removes the row (`deleteHistorialItem`/`deleteHistUbic`). Canceling edit (clicking the pencil again) restores `data-orig` instead of persisting.
